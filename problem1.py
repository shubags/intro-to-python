...
graph = plot_df.plot(x = 'income_band', y = 'rel_cum_freq');graph.axes.set_xticklabels(bands);
#vertical lines
graph.axvline(0, color = 'b', linestyle = '--'); graph.axvline(1, color = 'b', linestyle = '--');
#horizonal lines
...
#labels and title
graph.set_xlabel("Income Band"); graph.set_ylabel("Cumulative Frequency"); graph.set_title("Income Study")
...
income_df['ind_median_bin'] = (income_df['half_tot_households'] > income_df['prev_cum_freq']) & (income_df['half_tot_households'] < income_df['cum_freq'])
